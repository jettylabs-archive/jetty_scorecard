...
def _runner(env: SnowflakeEnvironment) -> tuple[float, str]:
    ...
    if env.masking_policies is None or len(env.masking_policies) == 0:
        ...
        details = render_string_template(
            """You have the following data masking policies in your environment:
<ul>
...
manually by running <code>"SELECT * FROM SNOWFLAKE.ACCOUNT_USAGE.POLICY_REFERENCES WHERE policy_kind = 'MASKING_POLICY'</code>.""",
